...
import snowman.level.objects.{Snow, Wall}
import snowman.level.{Coordinate, Level}
import snowman.solver.{BooleanVariable, IntegerVariable, Problem, VariableDeclaration}
...
class State(l: Level, val stateNumber: Int) {
    ...
    def addVariables(p: Problem): Unit = {
        p.add(VariableDeclaration(player.x))
        p.add(VariableDeclaration(player.y))

        for (b <- _balls) {
            p.add(VariableDeclaration(b.x))
            p.add(VariableDeclaration(b.y))
            p.add(VariableDeclaration(b.size))
        }
        for (v <- _reachableNodes) {
            p.add(VariableDeclaration(v))
        }
